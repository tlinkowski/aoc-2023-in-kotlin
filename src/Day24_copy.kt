...
fun main() {
    ...
    fun Velocity.normalize() =
        if (isZero()) this else listOf(dx, dy, dz).map { it.toLong() }.reduce(::gcd)
            .let { Velocity(dx / it, dy / it, dz / it) }
...
//    }
    ...
    fun Velocity.isColinear(o: Velocity): Boolean {
        val a = normalize()
        val b = o.normalize()
        return a == b || a == b.opposite()
    }

    fun Hailstorm.findParallelLinePairs() = hailstonePairs()
        .filter { (a, b) -> a.v.isColinear(b.v) }

    fun Hailstorm.candidateSegments() = hailstonePairs()
        .mapNotNull { (a, b) -> a.shortestSegment(b) }
        .sortedBy { s -> s.velocity().length() }

    fun Hailstorm.newOrigin() = Point3(
        x = hailstones.minOf { it.p.x },
        y = hailstones.minOf { it.p.y },
        z = hailstones.minOf { it.p.z }
    )

    fun Point3.transpose(o: Point3) = Point3(
        x = x - o.x,
        y = y - o.y,
        z = z - o.z
    )

    fun Hailstone.transpose(o: Point3) = copy(p = p.transpose(o))

    fun Hailstorm.transpose(o: Point3) = Hailstorm(hailstones.map { it.transpose(o) })
    ...
    fun part2(input: List<String>): Long {
        ...
        for (t in 0..10) {
            println("t=$t")
            hailstorm.hailstones.forEach {  h ->
                println(h.pointAt(t.toDouble()))
            }
        }

//        val rv = Velocity(-3.0, 1.0, 2.0)
//
//        hailstorm.hailstones
//            .map { it.v.dotProduct(rv) }
//            .forEach { println(it) }

//        val newOrigin = hailstorm.newOrigin()
//        println("new origin: $newOrigin")
//
//        val tHailstorm = hailstorm.transpose(newOrigin)
//        val three = tHailstorm.hailstones
//            .filter { it.p.x == 0.0 || it.p.y == 0.0 || it.p.z == 0.0 }
//        three.forEach { h -> println(h) }
//
//        val v = three.map { it.v }.reduce(Velocity::crossProduct).normalize()
//        println(v)

//        val parallel = hailstorm.findParallelLinePairs()
//            .onEach { println(it) }
//        println(parallel.size)

//        val p = hailstorm.candidateSegments()
//            .take(100)
//            .onEach { println("C: ${it.velocity().length()}")}
//            .firstNotNullOf { hailstorm.startPoint(it) }

        return 48L //p.x.roundToLong() + p.y.roundToLong() + p.z.roundToLong()
    }
